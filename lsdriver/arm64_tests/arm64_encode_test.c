...
int main(void)
{
    ...
    EXPECT_ENCODE("cbz w3", 0x34000803, arm64_encode_cbz(0x1000, 0x1100, false, 3, &instruction));
    EXPECT_ENCODE("cbnz x3", 0xB5000803, arm64_encode_cbnz(0x1000, 0x1100, true, 3, &instruction));
    EXPECT_ENCODE("tbz x5 bit 40", 0xB647F805, arm64_encode_tbz(0x1000, 0x0F00, 40, 5, &instruction));
    EXPECT_ENCODE("tbnz x5 bit 40", 0xB747F805, arm64_encode_tbnz(0x1000, 0x0F00, 40, 5, &instruction));
    ...
    EXPECT_ENCODE("str x30", 0xF9008BFE, arm64_encode_load_store_unsigned_offset(false, ARM64_ENCODE_LS_GPR64, 30, 31, 272, &instruction));
    EXPECT_ENCODE("ldr x17", 0xF9407E11, arm64_encode_load_store_unsigned_offset(true, ARM64_ENCODE_LS_GPR64, 17, 16, 248, &instruction));
    EXPECT_ENCODE("str q0", 0x3D800400, arm64_encode_load_store_unsigned_offset(false, ARM64_ENCODE_LS_SIMD128, 0, 0, 16, &instruction));
    EXPECT_ENCODE("str x16 pre", 0xF81F0FF0, arm64_encode_load_store_gpr64_pre_post_indexed(false, true, 16, 31, -16, &instruction));
    EXPECT_ENCODE("ldr x16 post", 0xF84107F0, arm64_encode_load_store_gpr64_pre_post_indexed(true, false, 16, 31, 16, &instruction));
    EXPECT_ENCODE("stp x0 x1", 0xA90207E0, arm64_encode_load_store_pair_gpr64(false, 0, 1, 31, 32, &instruction));
    EXPECT_ENCODE("ldp x28 x29", 0xA94E761C, arm64_encode_load_store_pair_gpr64(true, 28, 29, 16, 224, &instruction));
    ...
    EXPECT_ENCODE("msr nzcv", 0xD51B4209, arm64_encode_msr_register(9, 3, 3, 4, 2, 0, &instruction));
    ...
    expect_status("misaligned pair", arm64_encode_load_store_pair_gpr64(true, 0, 1, 2, 4, &instruction), -EINVAL);
