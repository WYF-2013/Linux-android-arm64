...
static void test_page_relative_relocation(void)
{
    ...
    expect_status("source cbnz", arm64_encode_cbnz(source_page + 8, source_page + 0xC0, true, 3, &source[2]), 0);
    expect_status("source tbz", arm64_encode_tbz(source_page + 12, source_page + 0x100, 40, 5, &source[3]), 0);
    ...
    expect_status("expected cbnz", arm64_encode_cbnz(ghost_page + 8, ghost_page + 0xC0, true, 3, &expected), 0);
    expect_word("page cbnz", output[2], expected);
    expect_status("expected tbz", arm64_encode_tbz(ghost_page + 12, ghost_page + 0x100, 40, 5, &expected), 0);
...
static void test_far_slots(void)
{
    ...
    expect_status("literal slot load", arm64_encode_load_store_unsigned_offset(true, ARM64_ENCODE_LS_GPR32, 6, 6, 0, &expected), 0);
